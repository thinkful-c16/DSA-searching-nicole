...
const linSearch = (array, value) => {
  let tries = 0;
  //loop through array of data
  for (let i = 0; i < array.length; i++) {
    //increment the tries each time 
    tries++;
    if (array[i] === value) {
      return `${value} was found in dataset after ${tries} tries`;
    }
  }
  return `After ${tries} item was not found`;
};

console.log(linSearch(dataset, 78));